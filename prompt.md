...
根据 `raw\lecxx.txt` 中的笔记内容，在 `lecxx` 文件夹内生成一份 LaTeX 文档。

## 具体要求

- **内容完善**：补全提到但未完整记录的要点；加入对于所有定理及性质的直观理解和证明，并提及提出的动机和原因；每个定理要求定义明确，不要有歧义；证明过程完整严谨，不要跳步或显然，不要省略步骤，不要打比方，不要默认我什么都会。
- **准确性**：修正笔记中不规范及错误的数学表示，笔记中提到的所有内容都需在文档中体现。
- **结构化**：分点清晰明了，保证笔记脉络连贯；在有多种选择时，选择最为广泛使用的数学符号表示和数学术语，且保持前后一致。
- **排版规范**：保证输出的 LaTeX 格式美观，缩进换行正确。定理/定义/性质/推论等部分请调用 `preamable_sep.tex` 中的样式模板。
